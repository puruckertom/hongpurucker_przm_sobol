...
system.time({
  ...
  for (Ite in 1:N_MC){
    ...
    Num_Tapp=25
    row_0=61
    TAPP=runif(N_MC, min=0.15, max=0.32)       #one application for all or different
    for (i in 1:Num_Tapp){
      row_t=row_0+(i-1)*1
    ...
    ####Management Factor##################
    USLEC1=runif(N_MC, min=0.59, max=0.63)       #establishment
    USLEC2=runif(N_MC, min=0.22, max=0.44)       #maturing
...
save.image(paste(pth,'/All_Var_20000_N.RData', sep=""))
##########################################################
